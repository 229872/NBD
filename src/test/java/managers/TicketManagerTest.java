...
//    }
    ...
    @Test
    public void collisionTest() throws WrongValueException {
        ...
        MovieManager movieManager2 = new MovieManager(movieRepository2);
        ...
        boolean isTaken1 = ticketManager1.isSeatTaken(seat, movieForClient1);
        boolean isTaken2 = ticketManager2.isSeatTaken(seat, movieForClient2);

        em1.getTransaction().begin();
        if(!isTaken1) {
            Ticket ticket1 = new Normal(basePrice, seat, client1, movieForClient1);
            em1.lock(movieForClient1, LockModeType.OPTIMISTIC_FORCE_INCREMENT);
            client1.addTicket(ticket1);
            em1.persist(ticket1);
        } else System.out.println("taken");
        em1.getTransaction().commit();

        em2.getTransaction().begin();
        if(!isTaken2) {
            Ticket ticket2 = new Normal(basePrice, seat, client2, movieForClient2);
            em2.lock(movieForClient2, LockModeType.OPTIMISTIC_FORCE_INCREMENT);
